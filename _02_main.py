...
#             print(" args.world_size = 1 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ...
        with amp.autocast(enabled=args.amp):  #16-bit precision 
            ...
            t_images = torch.cat((images_l, images_uw, images_us))  #Concatenates the given sequence of seq tensors in the given dimension
            ...
            t_loss_l = criterion(t_logits_l, targets.long())  # label dataset의 예측값 t_logit_l과 gt인 targets의 비교 
            ...
            s_images = torch.cat((images_l, images_us))  # studenet의 input image 생성,  labeled + unlabeled strong aug 으로 구성 
...
# https://discuss.pytorch.org/t/runtimeerror-expected-object-of-scalar-type-long-but-got-scalar-type-float-when-using-crossentropyloss/30542/2
            ...
            s_loss_l_old = F.cross_entropy(s_logits_l.detach(), targets_long)  # labeled data에 대한 student의 예측값 s_logit_l과 정답 targets과 비교
                                                                                # studenet의 loss 값을 cross_entropy로 계산 
