...
class MakeDataset():
    ...
    def main(self):
        ...
        with open('cnt.csv', 'w', newline="") as f:
            w = csv.writer(f)
            for k, v in class_data_num.items():
                w.writerow([k, v])
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        ...
        #             ).reset_index(drop=True)
        #-----------------------------------------------------------------------------------------------------------------------------     
        # 220126 그룹화 클래스별 갯수 추출
        df = df.groupby('ori_class_nm', group_keys=False).apply(lambda x : 
            x.sample(n=int(class_data_num[x.name]))).reset_index(drop=True)
        df.to_csv(os.path.join(save_set, 'filtered_df.csv'), index=False)
        new_df = df['class_nm'].value_counts()
        print(new_df)
        new_df.to_csv(os.path.join(save_set, 'group_cnt.csv'), index=False)
        #-----------------------------------------------------------------------------------------------------------------------------  
