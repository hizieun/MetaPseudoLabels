...
실행 : python _01_make_dataset.py
설명 : data/root/_data/denta 임시 데이터 생성 및 하위 연령별/성별 치아 학습 그룹화 데이터셋 생성
python make_dataset.py --data-path ../data/backup/ --req-sample y
'''
import os
import shutil
import pandas as pd
...
class MakeDataset():
    ...
    def arg_parser_add(self):
        ...
        # MPL 저장할 데이터셋명
        parser.add_argument('--data-name', default=f'age_cls_custom_{today}', type=str, help='dataset name')
        # MPL 학습 데이터셋 저장경로
    ...
    def main(self):
        ...
        path_list = [args.save_path, save_set, save_train, save_test]
